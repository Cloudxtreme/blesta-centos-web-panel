 ...
 *
 * The XML-API class allows for easy execution of Cwp XML-API calls.  The goal of this project is to create
 ...
 * @category Cwp
 * @package xmlapi
 * @copyright 2012 Cwp, Inc.
 * @license http://sdk.Cwp.net/license/bsd.html
 * @version Release: 1.0.13
 * @link http://twiki.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/XmlApi
...
class CwpApi
{
     ...
     * @param string $key The api key used to authenticate
    ...
    public function __construct()
    {
        ...
        if ((defined('CWPAPI_DEBUG')) && (CWPAPI_DEBUG == '1')) {
            $this->debug = true;
        }
    }
    ...
    /**
     * Set the key to be autenticated with
     *
     * This will set the key to be authenticated with
     *
     * @param string $key to authenticate with
     */
    public function set_key($key)
    {
        if ((empty($key)) || (strlen($key) == 0)) {
            throw new Exception('Invalid key set.');
        }
        $this->key = $key;
    }
    ...
    {
        if ((empty($host)) || (strlen($host) == 0)) {
            throw new Exception('Invalid host set.');
        }
    ...
    public function api_query($function, $vars = array())
    {
        // Check to make sure all the data needed to perform the query is in place
        if (!$function) {
            throw new Exception('api_query() requires a function to be passed to it');
        }

        if ($this->host == null) {
            throw new Exception('no host has been set');
        }
        ...
        if ($function == null) {
            throw new Exception('no function has been set');
        }

        // Allowed functions
        $allowed_functions = array("validate_api", "account_new", "account_remove", "account_suspend", "account_unsuspend");
        ...
        $query_type = '/api/';
        $query_type = $query_type . "?key=" . $this->key;

        $args = http_build_query($vars, '', '&');
        $url = $this->protocol . '://' . $this->host . ':' . $this->port . $query_type;
        ...
        // Validate the api access
        if ($function == "validate_api") {
            // No args passed nothing to do really
        }

        // Create an account
        if ($function == "account_new") {
            // Check the required variables to create an account
            // &api=account_new
            // &domain=cwptest.com
            // &username=cwptest
            // &password=gew42f4
        ...
        */
        ...
        //
        // Lets parse the response, cwp should use json or xml but no they use plain simple text.
        //
        // Possible identified responses
        // Allowed IP's not set or you don't have access!
        // Key not set or invalid!
        // Failed to include API File!!!
        // Check All Fields (only lower caps are allowed and no special characters)!
        // Username XXXXXX already exists in database!
        //

        // I know this is not the best method to test this out
        if (($response == "Failed to include API File!!!") && ($function == "validate_api")) {
            return 0; // Valid api (or at least passed the key check).
        }

        // Not implemented method
        if (($response == "Failed to include API File!!!")) {
            return 1; // This function was not implemented.
        }

        // Check for wrong ip access
        if ($response == "Allowed IP's not set or you don't have access!") {
            return 2; // No access do api
        }

        // Check for a bad api key
        if ($response == "Key not set or invalid!") {
            return 3; // Invalid api key
        }

        // Check for missing fields or wrong fields
        if ($response == "Check All Fields (only lower caps are allowed and no special characters)!") {
            return 4; // Tried to submit invalid fields.
        }

        // Check for an already existing user
        if (strpos($response, "already exists in database!") == true) {
            return 5; // Already existing user
        }
    ...
    private function fopen_query($url)
    {
    ...
    {
        $result = $this->api_query("validate_api", array());
        return $result;
    }
    ...
    */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/CreateAccount XML API Call documentation
     */
    ...
    */
    ...
    /**
     * Change a Cwp Account's Password
     *
     * This function will allow you to change the password of a Cwp account
     *
     * @param string $username The username to change the password of
     * @param string $pass The new password for the Cwp Account
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ChangePassword XML API Call documentation
     */
     ...
     *
     * @param string $username The username of the Cwp account to modify
     * @param int $bwlimit The new bandwidth limit in megabytes
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/LimitBandwidth XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ListAccounts XML API Call documentation
     */
     ...
     * @param array $args the new values for the modified account (see {@link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ModifyAccount modifyacct documentation})
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ModifyAccount XML API Call documentation
     */
    ...
    /**
     * Edit a Cwp Account's Quota
     *
     * This call will allow you to change a Cwp account's quota
     *
     * @param string $username The username of the account to modify the quota.
     * @param int $quota the new quota in megabytes
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/EditQuota XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ShowAccountInformation XML API Call documenation
     */
     ...
     *
     * This function will suspend the specified Cwp users account.
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/SuspendAccount XML API Call documentation
     */
     ...
     *
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ListSuspended XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/TerminateAccount
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/UnsuspendAcount XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ChangePackage XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ViewPrivileges XML API Call documentation
     */
     ...
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/DomainUserData
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/SetSiteIp XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/AddDNSZone XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/AddZoneRecord XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/EditZoneRecord XML API Call documentation
     * @see dumpzone()
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/GetZoneRecord XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/DeleteDNSZone XML API Call documentation
     */
     ...
     *
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ListDNSZone XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ListOneZone XML API Call documentation
     * @see editdnsrecord()
     * @see getdnsrecord()
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/LookupIP XML API Call documentation
     */
     ...
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/RemoveZone XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ResetZone XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/AddPackage XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/DeletePackage XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/EditPackage XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ListPackages XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/AddResellerPrivileges XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/CreateResellerACLList XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ListCurrentResellerACLLists XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ListResellerAccounts XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ListResellersAccountsInformation XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/RemoveResellerPrivileges XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/SetResellersACLList XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/TerminateResellerandAccounts XML API Call documentation
     *
     **/
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/SetResellerIps XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/SetResellerLimits XML API Call documentation
     *
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/SetResellerMainIp XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/SetResellerPkgLimit XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/SuspendReseller XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/UnsuspendReseller XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/AcctCounts XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/SetResellerNameservers XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/DisplayServerHostname XML API Call documentation
     */
    ...
    /**
     * Get the version of Cwp running on the server
     *
     * This function will return the version of Cwp/WHM running on the remote system
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/DisplayCwpWHMVersion XML API Call documentation
     */
     ...
     *
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/LoadAvg XML API Call documentation
     */
     ...
     *
     * This function will return a list of available langauges for the Cwp interface
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/GetLangList XML API Call documentation
     *
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/RebootServer XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/AddIPAddress XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/DeleteIPAddress XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/SetHostname XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/SetResolvers XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ShowBw XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/RestartService XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ServiceStatus XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ConfigureService XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/FetchSSL XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/GenerateSSL XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/InstallSSL XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ListSSL XML API Call documentation
     */
     ...
     * @return mixed
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/CallingAPIFunctions XML API Call documentation
     * @link http://docs.Cwp.net/twiki/bin/view/DeveloperResources/ApiRef/WebHome API1 & API2 Call documentation
     * @link http://docs.Cwp.net/twiki/bin/view/DeveloperResources/ApiBasics/CallingApiOne API1 Documentation
     */
    public function api1_query($user, $module, $function, $args = array())
    {
        ...
        $cpuser = 'Cwp_xmlapi_user';
        $module_type = 'Cwp_xmlapi_module';
        $func_type = 'Cwp_xmlapi_func';
        $api_type = 'Cwp_xmlapi_apiversion';

        if ($this->get_output() == 'json') {
            $cpuser = 'Cwp_jsonapi_user';
            $module_type = 'Cwp_jsonapi_module';
            $func_type = 'Cwp_jsonapi_func';
            $api_type = 'Cwp_jsonapi_apiversion';
        }
        ...
        return $this->xmlapi_query('Cwp', $call);
    }
    ...
    ####
    # Cwp API1 functions
    # Note: A Cwp account username is required
    # Some Cwp features must be enabled to be able to use some function (f.e. park, unpark)
    ####
     ...
     *
     * This function allows you to call an API2 function, this is the modern API for Cwp and should be used in preference over
     * API1 when possible
     *
     ...
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/CallingAPIFunctions XML API Call documentation
     * @link http://docs.Cwp.net/twiki/bin/view/DeveloperResources/ApiRef/WebHome API1 & API2 Call documentation
     * @link http://docs.Cwp.net/twiki/bin/view/AllDocumentation/AutomationIntegration/ApiTwo Legacy API2 Documentation
     * @link http://docs.Cwp.net/twiki/bin/view/DeveloperResources/ApiBasics/CallingApiTwo API2 Documentation
     */
    ...
    public function api2_query($user, $module, $function, $args = array())
    {
        ...
        return $this->xmlapi_query('Cwp', $args);
    }
    ...
    # Cwp API2 functions
    # Note: A Cwp account username is required
    # Some Cwp features must be enabled to be able to use some function
    ####
    ...
    // This API2 function allows you to list ftp-users associated with a Cwp account including disk information.
    ...
    // This API2 function allows you to list ftp-users associated with a Cwp account.
